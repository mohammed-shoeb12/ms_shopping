...
class ProductView(View):
	def get(self, request):
		...
		cosmatic = Product.objects.filter(category='C')
		other = Product.objects.filter(category='O')
		tranding = Product.objects.filter(category='T')
		if request.user.is_authenticated:
			totalitem = len(Cart.objects.filter(user=request.user))
		return render(request, 'app/home.html', {'topwears':topwears, 'bottomwears':bottomwears, 'mobiles':mobiles, 'totalitem':totalitem, 'cosmatic':cosmatic, 'other':other,'tranding':tranding })
...
def top(request, data=None):
	...
	if data==None :
			...
			tops = Product.objects.filter(category='TW').filter(discounted_price__lt=300)
	elif data == 'above':
			tops = Product.objects.filter(category='TW').filter(discounted_price__gt=300)
...
def bottom(request, data=None):
	...
	if data==None :
			...
			bottoms = Product.objects.filter(category='BW').filter(discounted_price__lt=400)
	elif data == 'above':
			bottoms = Product.objects.filter(category='BW').filter(discounted_price__gt=300)
	return render(request, 'app/bottom.html', {'bottoms':bottoms, 'totalitem':totalitem})
